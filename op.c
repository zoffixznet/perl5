    ...
    if (sv) {
        if (sv->op_type == OP_RV2SV) {	/* symbol table variable */
            ...
            if (!pushmark || pushmark->op_type != OP_PUSHMARK) {
                /* diag_listed_as: Can't use %s for loop variable */
                Perl_croak(aTHX_ "Can't use list for loop variable (not a pushmark)");
            }
            first_padsv = OpSIBLING(pushmark);
            if (!first_padsv || first_padsv->op_type != OP_PADSV) {
                /* diag_listed_as: Can't use %s for loop variable */
                Perl_croak(aTHX_ "Can't use list for loop variable (not a padsv)");
            }
            ...
            do {
                if (!padsv || padsv->op_type != OP_PADSV) {
                    /* diag_listed_as: Can't use %s for loop variable */
                    Perl_croak(aTHX_ "Can't use list for loop variable (not a pad2v at %lu)", how_many_more);
                }
                ++how_many_more;
                if (padsv->op_targ != padoff + how_many_more) {
                    /* diag_listed_as: Can't use %s for loop variable */
                    Perl_croak(aTHX_ "Can't use list for loop variable (padsv at %lu targ is %lu, not %lu)", how_many_more, padsv->op_targ, padoff + how_many_more);
                }
