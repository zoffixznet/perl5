/*    locale.c
 * XXX other workspace has stashes that we need to potentially incorporate, including the NL_LOCALE for setlocale("" no querylocale)
 * Comment that querylocale_ needs savepv(); consider having it return SAVEFREEPV
...
#  define toggle_locale_i(a,b)	S_toggle_locale_i(aTHX_ a,b, __LINE__)
#  define toggle_locale_c(cat, locale)  toggle_locale_i(cat##_INDEX_, locale)
#define restore_toggled_locale_i(a,b)	                                    \
                             S_restore_toggled_locale_i(aTHX_ a,b, __LINE__)
...
S_less_dicey_setlocale_r(pTHX_ const int category, const char * locale)
{
    ...
    PORCELAIN_SETLOCALE_LOCK;   /* XXX rename STDIZED_SETLOCALE_LOCK */
...
#  ifdef HAS_GLIBC_LC_MESSAGES_BUG
    ...
    if (old_messages_locale) {
        /* XXX can this destroy new_locale? */
...
wchar_t *
S_Win_byte_string_to_wstring(UINT code_page, const char * byte_string)
{
    wchar_t *wstring;
...
S_wrap_wsetlocale(pTHX_ int category, const char *locale) {
    ...
    return Win_wstring_to_utf8_string(wresult);
...
#  endif
    ...
    for (i = 0; i < LC_ALL_INDEX_; i++) {
        const char * env_name = PerlEnv_getenv(category_names[i]);
        if (env_name && strNE(env_name, "")) {
            const char * discard
                            = S_wrap_wsetlocale(aTHX_ categories[i], env_name);

            DEBUG_Lv(PerlIO_printf(Perl_debug_log, "%s\n",
                      setlocale_debug_string_i(i, env_name, "not captured")));
...
#  endif
        ...
        DEBUG_L(PerlIO_printf(Perl_debug_log, "%s\n",
                             setlocale_debug_string_r(category, NULL, retval)));
    ...
    {
        DEBUG_L(PerlIO_printf(Perl_debug_log,
               "Perl_setlocale(%s, '%s'): Already in '%s': no action taken\n",
               category_names[cat_index], locale, locale));
...
#  define UTF8NESS_UNKNOWN  -1 /* mnemonic value for 'known_utf8' */
    ...
    DEBUG_Lv(PerlIO_printf(Perl_debug_log, "Entering get_locale_string_utf8ness_i; locale=%s, index=%u(%s), string=%s, known_utf8=%d\n",
                                           locale, cat_index, category_names[cat_index], _byte_dump_string((U8 *) string, strlen(string), 0), known_utf8));
    if (string == NULL) {
        return 0;
    }
...
#  if defined(HAS_SOME_LANGINFO) || defined(HAS_MBTOWC) || defined(HAS_MBRTOWC)
...

    {
        ...
        /* 'locale' can be pointing to a buffer destroyed in the call, so save
         * it.  This may only happen when the input is NULL, so could check for
         * that XXX */
...
localeconv() returns data based on the current underlying locale.  That means
that the numeric locale may need to be toggled.  This is done if needed in the
called function.

localeconv() returns strings based on both the LC_NUMERIC and LC_MONETARY
locales.  These are theoretically independent, but some platforms return
mojibake if LC_CTYPE is not in the same locale as the category controlling the
given string.  (khw suspects that it is actually looser than this: that it may work if both are single-byte locales,
controlling locale for the strings returned by localeconv() (or more likely, if
one is multi-byte and the other isn't).  khw suspects that no one really thought about the possibility of
*/
...
#  ifdef WIN32
#    define BROKEN_UTF8_LOCALECONV
#  endif

#  ifdef BROKEN_UTF8_LOCALECONV
#    ifdef HAS_LOCALECONV_L
#      error Code assumes localeconv_l() isnt on Windows
#    endif

STATIC const char *
S_fixup_broken_localeconv_utf8(pTHX_ const char * input,
                                     const bool is_locale_utf8,
                                     bool *need_to_free)
{
    const wchar_t * wide_input;
    const char * utf8_result;

    /* There is a bug on most (if not all) Windows systems, in which some
     * locales that are marked as UTF-8, and mostly behave as UTF-8, do not
     * return the proper UTF-8-encoded strings in localeconv().  Instead, they
     * apparently return based on the ANSI code page.
     *
     * This function works around that by, in UTF-8 locales when the string
     * isn't legal UTF-8, converting the ACP version to wchar_t strings, and
     * thence to UTF-8. */

    assert(*need_to_free == FALSE);

    if (! is_locale_utf8) {
        return input;
    }

    if (LIKELY(is_utf8_string((U8 *) input, strlen(input)))) {
        return input;
    }

    *need_to_free = TRUE;

    wide_input = Win_byte_string_to_wstring(CP_ACP, input);
    utf8_result = Win_wstring_to_utf8_string(wide_input);
...
#    define fixup_broken_localeconv_utf8(s, utf8, free)                       \
                          S_fixup_broken_localeconv_utf8(aTHX_ s, utf8, free)
#  else     /* A no-op on platforms that don't have this bug */
#    define fixup_broken_localeconv_utf8(s, utf8, free)                       \
                                                   (PERL_UNUSED_ARG(utf8), s)
#  endif
#  if ! (defined(USE_POSIX_2008_LOCALE) && defined(HAS_LOCALECONV_L))

STATIC HV *
S_lock_and_run_localeconv(pTHX_
            HV * (*copy_localeconv)(pTHX_ const struct lconv *, int, int),
            const int numeric_locale_is_utf8,
            const int monetary_locale_is_utf8)
{
    /* This function calls localeconv() and saves the result.  It uses
     * 'copy_localeconv()' to do the saving.
     *
     * At entry, the caller must have set the necessary categories to the
     * proper locale(s).
     *
     * This function just wraps the appropriate locks/unlocks around the call
     * and stash.  There are so many locks because localeconv() deals with two
     * categories, (with a third drawn in) and returns in a single global
     * static buffer.  Some locks might be no-ops on this platform, but others
     * might actually do something.  We need to lock if any one isn't a no-op.
     *
     * And on boxes where localeconv() only looks at the global locale when run
     * under per-thread locales, it also saves the state, switches to the
     * global locale, does the operation and save, then restores.
     *
     * A complication is for the broken localeconv() on Windows with
     * thread-safe locales prior to VS 15.  It looks at the global locale
     * instead of the thread one.  As a work-around, we toggle to the
     * global locale; populate the return; then toggle back.  We have to
     * use LC_ALL instead of the individual categories because of another
     * bug in Windows.  This code takes up most of this function.
     *
...
S_my_localeconv(pTHX_ const int item)
{
    ...
    HV * (*copy_localeconv)(pTHX_ const struct lconv *, int, int);
    unsigned needed_categories = 0;

    /* Both NUMERIC and MONETARY control localeconv().  In some calls to this
     * function, only one of them is needed.  A bit is set in
     * 'needed_categories' for each one that is needed. */
#    define NEED_NUMERIC  1 << 0
#    define NEED_MONETARY 1 << 1
     ...
     *
     * On input, LC_MONETARY, if used, needs to be set correctly.
     *
     * There are two use cases:
     * 1) Called from POSIX::locale_conv().  This returns the lconv() struct
     *    copied to a hash, based on the current underlying locale.
     ...
     * from each of the various implementations.
     *
     * The libc localeconv() functions tend to have had bugs, especially under
     * Windows.  Code is still here to work around some of these.
     *
     * When dealing with strings, some platforms require LC_CTYPE to be in the
     * same locale as the category controlling the string (or perhaps it is
     * that one can't combine single-byte CTYPE with multi-byte controlling
     * category, or vice versa).  Mojibake can otherwise result.  So the code
     * below, switches CTYPE to be the same category as the controlling one.
     * But localeconv() has two controlling categories, NUMERIC and MONETARY.
     * They could be set to incompatible values.  This code will switch CTYPE
     * to MONETARY.  The reason is that there are far fewer locales in the
     * world where mojibake could be a problem, namely some of the Arabic ones.
     * Everywhere else uses a dot or a comma as the radix character; there may
     * be others that use some sort of space for the thousands separator, but
     * mostly it is empty, or an ASCII SPACE.  */
...
#  ifdef USE_LOCALE_NUMERIC
        ...
        needed_categories |= NEED_NUMERIC;

#  endif
#  ifdef USE_LOCALE_MONETARY

        {
            const char * monetary_locale = savepv(querylocale_c(LC_MONETARY));

            monetary_locale_is_utf8 = is_locale_utf8(monetary_locale);
            Safefree(monetary_locale);
            needed_categories |= NEED_MONETARY;
        }
...
#  ifndef HAS_SOME_LANGINFO
        ...
        /* To avoid some extra unused parameters, we repurpose the parameter
         * names which get passed to get_nl_item_from_localeconv()
         *
         * 'numeric_locale_is_utf8' instead means 'item'
         * 'monetary_locale_is_utf8' instead is a boolean as to if the locale
         *      corresponding to 'item' is UTF-8 or not.  This is only used on
         *      platforms with a broken localeconv(); set it to a don't care
         *      value otherwise
         *
         * Hopefully, this isn't too confusing; it allows for not having to
         * have some preprocessor-controlled function declarations. */

        numeric_locale_is_utf8 = item;

#    ifndef BROKEN_UTF8_LOCALECONV  /* XXX */

        monetary_locale_is_utf8 = UTF8NESS_UNKNOWN;
...
#      ifdef USE_LOCALE_NUMERIC
            ...
            monetary_locale_is_utf8 = is_locale_utf8(PL_numeric_name);
            needed_categories |= NEED_NUMERIC;
 ...
 * implementations when we can use locale_conv_l().  These are the simplest
 * cases, preferred because we assume the function is thread-safe, hence no
 * locking is needed.  The function is not in the POSIX standard, but currently
 * exists on Darwin and *BSD-derived platforms.  The more complete *BSD man
 * pages indicate it is thread-safe; while Darwin's less detailed man page
 * currently does not.  But any reasonable implementation will store the result
 * with the passed-in locale, making it likely thread-safe.
 */
#  if defined(USE_POSIX_2008_LOCALE) && defined(HAS_LOCALECONV_L)
#    ifndef HAS_SOME_LANGINFO
#      error Code assumes nl_langinfo() is available
#    endif
#    ifndef USE_LOCALE_MONETARY

    {
        locale_t numeric    /* No MONETARY; use NUMERIC for everything */
                        = newlocale(LC_ALL_MASK, PL_numeric_name, (locale_t) 0);

        retval = copy_localeconv(aTHX_ localeconv_l(numeric),
...
/*--------------------------------------------------------------------------*/
#    else   /* Below, paying attention to MONETARY */
#      ifndef USE_LOCALE_CTYPE
#        ifndef USE_LOCALE_NUMERIC

    /* No CTYPE nor NUMERIC, use the base MONETARY */
    return copy_localeconv(aTHX_ localeconv_l(use_curlocale_scratch),
                                 numeric_locale_is_utf8,
                                 monetary_locale_is_utf8);

/*--------------------------------------------------------------------------*/
#        else   /* No CTYPE, but both NUMERIC and MONETARY */

    {
        /* Start with MONETARY */
        const locale_t cur = use_curlocale_scratch();

        /* Copy it so it won't get destroyed, and add in NUMERIC */
        locale_t with_numeric = duplocale(cur);
        with_numeric
                    = newlocale(LC_NUMERIC_MASK, PL_numeric_name, with_numeric);
...
/*--------------------------------------------------------------------------*/
#        endif
#      else     /* Below, paying attention to CTYPE and MONETARY */

    {
        /* Start with MONETARY */
        const locale_t cur = use_curlocale_scratch();

        /* Copy it so it won't get destroyed, and add in CTYPE */
        locale_t with = duplocale(cur);
        with = newlocale(LC_CTYPE_MASK, querylocale_c(LC_MONETARY), with);
...
#        endif
        ...
        retval = copy_localeconv(aTHX_ localeconv_l(with),
                                       numeric_locale_is_utf8,
                                       monetary_locale_is_utf8);
        freelocale(with);
...
/*--------------------------------------------------------------------------*/
#      endif    /* Both CTYPE and no CTYPE */
#    endif    /* Both MONETARY and no MONETARY */
#  else   /* Above, localeconv_l(); below, plain localeconv() */

    /* For regular localeconv(), we set up the locales of each affected
     * category, using the passed-in mask; then call localeconv(); stash
...
#    ifndef USE_LOCALE_MONETARY

    {
        ...
        orig_numeric_locale = toggle_locale_c(LC_NUMERIC, PL_numeric_name);

#      ifdef USE_LOCALE_CTYPE
        orig_ctype_locale = toggle_locale_c(LC_CTYPE, PL_numeric_name);
#      else
        PERL_UNUSED_VAR(orig_ctype_locale);
#      endif

        retval = S_lock_and_run_localeconv(aTHX_ copy_localeconv,
                                           numeric_locale_is_utf8,
                                           monetary_locale_is_utf8);
#      ifdef USE_LOCALE_CTYPE
...
#      ifdef USE_LOCALE_CTYPE
        ...
        orig_ctype_locale = toggle_locale_c(LC_CTYPE, monetary_locale;
        Safefree(monetary_locale);
...
/*--------------------------------------------------------------------------*/
#    else   /* Below, paying attention to both NUMERIC and MONETARY */
...
#      endif
        ...
        if (needed_categories & NEED_NUMERIC) {
            orig_numeric_locale = toggle_locale_c(LC_NUMERIC, PL_numeric_name);

            /* If we don't have to fix up LC_MONETARY, set everything to
             * LC_NUMERIC, so as to give localeconv() the least chance to be
             * confused */
            if (! (needed_categories & NEED_MONETARY)) {
                orig_monetary_locale = toggle_locale_c(LC_MONETARY, PL_numeric_name);
...
#      endif
        ...
        if (needed_categories & NEED_MONETARY) {
            ...
            /* We should already be in the underlying MONETARY locale, so no
             * need to switch that.  But CTYPE needs to be in that to prevent
             * potential mojibake on some systems.  If MONETARY and NUMERIC
             * differ, monetary wins, as almost everywhere the numeric return
             * string will be in ASCII, so immune from mojibake.  Only some
             * Arabic speaking countries have a non-ASCII radix */
            const char * monetary_name = savepv(querylocale_c(LC_MONETARY));

#      ifdef USE_LOCALE_CTYPE
            orig_ctype_locale = toggle_locale_c(LC_CTYPE, monetary_name);
#      endif

            /* If LC_NUMERIC doesn't matter, set everything to LC_MONETARY,
             * so as to give localeconv() the least chance to be confused */
            if (! (needed_categories & NEED_NUMERIC)) {
                orig_numeric_locale = toggle_locale_c(LC_NUMERIC, monetary_name);
            }

            Safefree(monetary_name);
        }
...
#  endif
    ...
    DEBUG_Lv(PerlIO_printf(Perl_debug_log,
                           "Entering populate_localeconv; %s\n",
                           get_LC_ALL_display()));
...
#  endif
        ...
        while (strings->name) {
            ...
            if (value && *value) {
                ...
                value = fixup_broken_localeconv_utf8(value,
                                                     locale_is_utf8,
                                                     &need_to_free);
                /* Only make UTF-8 if required to */
                is_utf8 = (2 == (get_locale_string_utf8ness_i(locale,
                                                              cat_index,
                                                              value,
                                                              locale_is_utf8)));
                                    ...
                                    const int locale_is_utf8)
{
    ...
    DEBUG_Lv(PerlIO_printf(Perl_debug_log,
                           "Entering get_nl_item_from_localeconv; item=%d,"
                           " locale_is_utf8=%d; %s\n",
                           item, locale_is_utf8, get_LC_ALL_display()));
    ...
    temp = fixup_broken_localeconv_utf8(temp, locale_is_utf8, &need_to_free);
    retval = Perl_newSVpvf(aTHX_ "%s%s", prefix, temp);
                ...
                int * utf8ness)
{
    ...
    /* If 'locale' is NULL, it means we know that the current locale for the
     * thread for the given category is the one we want.  The function could
     * have been written so the caller always had to switch to that locale
     * before calling us.  But if we are using nl_langinfo_l(), it is easy to
     * create a new locale object if necessary, without having to do any
     * switch, and tht is preferable to avoid unnecessary switching. */
    ...
    DEBUG_Lv(PerlIO_printf(Perl_debug_log,
            ...
            if (locale == NULL) {
                ...
                PerlIO_printf(Perl_debug_log, "explicit locale %s\n", locale);
            });
/*--------------------------------------------------------------------------*/
...
#  elif defined(HAS_NL_LANGINFO) /* nl_langinfo() is available.  */
    ...
    {
        const char * orig_category_locale = NULL;

        if (locale != NULL) {
            orig_category_locale = toggle_locale_i(cat_index, locale);
        }
        ...
        restore_toggled_locale_i(cat_index, orig_category_locale);
    }
...
#  else
...

    {
        const char * orig_category_locale = NULL;
        const char * orig_CTYPE_locale = NULL;
        bool need_to_free_locale = FALSE;

        /* Almost all the items will have ASCII return values.  Set that here,
...
#    ifdef USE_LOCALE_CTYPE
        ...
        /* Some platforms don't deal well with non-ASCII strings in locale
         * category X when LC_CTYPE is in category Y, and Y ≠  X.  (Actually it
         * is probably when X is UTF-8 and LC_CTYPE isn't, or vice versa).
         * There is explicit code to bring the categories into sync.  This
         * doesn't seem to be a problem with nl_langinfo(), so that
         * implementation doesn't currently worry about it.  But it is a
         * problem on Windows boxes, which don't have that function. */

        if (   cat_index != LC_CTYPE_INDEX_
            && strNE(locale, querylocale_c(LC_CTYPE)))
        {
...
#    ifdef USE_LOCALE_NUMERIC
            ...
            /* We have taken pains to make sure we are toggled into the
             * underlying numeric locale here. */
            assert(cat_index != LC_NUMERIC_INDEX_);

#    endif
#   endif

        /* Here, we are in the locale we want information about, and LC_CTYPE
         * is also set to that locale */
        switch (item) {
          default:
          ...
          case THOUSEP:
            {
                ...
                retval = save_to_buffer(SvPV_nolen(string),
...
#    endif
          ...
          case MON_9: case MON_10: case MON_11: case MON_12:

            {
                 ...
                 * first day of the week.  Since we're only getting one thing
                 * at a time, it all works */
                 ...
                 * for such scripts, as we can infer alt-1, alt-2, ....  But
                ...
                if (return_format) {
                    ...
                    /* If to return the format, not the value, overwrite the
                     * buffer with it.  But some strftime()s will keep the
                     * original format if illegal, so change those to "" */
          ...
          case CODESET:
            if (isNAME_C_OR_POSIX(querylocale_c(LC_CTYPE))) {
                retval = C_codeset;
                break;
            }
...
#    endif
             ...
             * (correctly) use that for the code set. */
...
#    if defined(HAS_MBTOWC) || defined(HAS_MBRTOWC)
                ...
                /* These functions weren't in the published C89 standard, but
                 * were added soon after, so that many sources consider them to
                 * be C89, and are likely available in a compiler that claims
                 * to support C89.
                 *
                 * Other functions that one could use are inverse ones, like
                 * wctomb(), and to a lesser extent MB_CUR_MAX.  But these all
                 * entered the Standard at the same time as the ones we do use,
                 * so it is extremely likely that all or none are available on
                 * this platform */

                /* If libc mbtowc() evaluates the bytes that form the
                 * REPLACEMENT CHARACTER as that Unicode code point, this has
                 * to be a UTF-8 locale */
...
#      ifdef USE_LOCALE_MONETARY
         ...
         /* Can't use this method unless localeconv() is available, as
          * otherwise it will infinitely recurse */
#        ifndef BROKEN_UTF8_LOCALECONV

                /* First try looking at the currency symbol (via a recursive
                 * call) to see if it disambiguates things.  Often that will be
                 * in the native script, and if the symbol isn't legal UTF-8,
                 * we know that the locale isn't either. */
                (void) my_langinfo_c(CRNCYSTR, LC_MONETARY, locale,
                                     &scratch_buf,
                                     &scratch_buf_size, &locale_is_utf8);
#        endif
#      endif
#      ifdef USE_LOCALE_TIME

                /* If that didn't rule out being UTF-8, we look at LC_TIME
                 * entries, like the names of the months or weekdays.  We quit
                 * after we find several that are all legal UTF-8 or the first
                 * one that is illegal */
                if (locale_is_utf8 != 0) {
                    int this_is_utf8;
                    unsigned int i;
                    unsigned int valid_utf8 = 0;
                     ...
                     * locales, but by doing it here, we avoid repeated
                     * switching in the loop */
                    ...
                    /* Here we have gone through what we consider are enough
                     * LC_TIME elements.  If at least one isn't legal UTF-8,
                     * locale_is_utf8==0; if all were ASCII, locale_is_utf8==1;
                     * otherwise locale_is_utf8==2. */
                }
...
#      endif    /* LC_TIME */
                ...
                /* Here, if the locale is UTF-8, we have likely discovered that
                 * fact, provided it doesn't use '$' for currency and we have
                 * access to LC_MONETARY, or isn't English, and we have access
                 * to LC_TIME. */
                if (locale_is_utf8 == 2) {
                    retval = "UTF-8";
                    break;
                ...
                /* The other locale categories are not likely to be of further
                 * help:
                 *
                 * LC_NUMERIC   Only a few locales in the world have a
                 *              non-ASCII radix or group separator.
                 * LC_CTYPE     On most platforms, the strict C89 functions
                 *              that deal with this category don't handle
                 *              non-ASCII in multi-byte locales, and even if
                 *              they did, they wouldn't be of any help in
                 *              distinguishing between UTF-8 and 8859-1.
                 * LC_MESSAGES  The strings returned from strerror() would seem
                 *              likely candidates, but experience has shown
                 *              that many systems don't actually have
                 *              translations installed for them.  They are
                 *              instead always in English, so everything in
                 *              them is ASCII, which is of no help to us.  A
                 *              Configure probe could possibly be written to
                 *              see if this platform has non-ASCII error
                 *              messages.  But given the fact that we would be
                 *              doing this only on compilers that haven't been
                 *              updated in decades, those are systems that
                 *              wouldn't be expected to have translations
                 *              anyway. */
...
#    endif    /* ! mbtowc() */
                 ...
                 *
                 * The only remaining option that khw managed to figure out is
                 * that not infrequently the locale name contains the code set
                 * as a suffix, like "foo.8859-6".
                 *
                 * This is very much less than ideal; there may very well be no
                 * such suffix, and at other times they even lie.
                 *
                ...
                /* But we do have options for checking if a code set claiming
                 * to be UTF-8 actually is. */
                if (is_codeset_name_UTF8(retval)) {

#    if defined(HAS_MBTOWC) || defined(HAS_MBRTOWC)

                    /* If either of these functions is available on the
                     * platform, we wouldn't have gotten here if the locale had
                     * acted like it was UTF-8. */
#    else
                    /* If we found an illegal UTF-8 sequence, this can't be a
                     * UTF-8 locale
                     *
                     * Functions like isalnum() could be used to rule out
                     * UTF-8, if a sufficient number of byte inputs to them
                     * returned results unexpected both in ASCII and 8859-1
                     * locales.  khw currently believes it isn't worth the
                     * bother to loop through the possible bytes, checking. */
                        ...
                        const line_t caller_line)
{
    ...
    DEBUG_Lv(PerlIO_printf(Perl_debug_log,
             "(%d): toggle_locale_i: index=%d(%s), wanted=%s, actual=%s\n",
    ...
    DEBUG_Lv(PerlIO_printf(Perl_debug_log, "(%d): %s locale switched to %s\n",
                                 ...
                                 const line_t caller_line)
{
    ...
    if (restore_locale == NULL) {
        DEBUG_Lv(PerlIO_printf(Perl_debug_log,
                               "(%d): No need to restore %s\n",
                               caller_line, category_names[cat_index]));
        return;
    }

    DEBUG_Lv(PerlIO_printf(Perl_debug_log,
                           "(%d): %s restoring locale to %s\n",
                           caller_line, category_names[cat_index],
                           restore_locale));
...
/* my_strerror() returns a newly malloc'd copy of the text of the error message
...
#  ifdef aTHX_
    ...
    return Perl_form(aTHX_
                     "%s:%d:%p: setlocale(%s(%d), %s%s%s) returned %s%s%s\n",
                     __FILE__, line, aTHX_
...
#  else
    ...
    return Perl_form(aTHX_
                     "%s:%d:0: setlocale(%s(%d), %s%s%s) returned %s%s%s\n",
                     __FILE__, line,
                     category_names[cat_index], categories[cat_index],
                     locale_quote, locale, locale_quote,
                     retval_quote, retval, retval_quote);
...
Perl_thread_locale_init(pTHX)
{
     ...
     *
     * This is a no-op unless per-thread locales are in effect, as it would be
     * changing the global locale.  Instead, a new thread relies on the copying
     * in sv.c of the state of the parent thread to initialize the new thread.
